...
template <typename T> struct IndexedRowValue {
  T val;
  std::size_t index;
};

template <typename T> struct TopTwo {
  IndexedRowValue<T> max;
  IndexedRowValue<T> almost_max;
};

template <typename T> constexpr TopTwo<T> sorted_first_two(T v1, T v2) {
...
constexpr T best_path_value_through_element(TopTwo<T> const &top_last_row,
                                            T val, std::size_t col) {
  return top_last_row.max.index != col ? top_last_row.max.val + val
...
constexpr TopTwo<T> find_best_paths_for_row(TopTwo<T> const &top_last_row,
                                            ...
                                            Matrix const &input) {
  auto path_0 =
      best_path_value_through_element(top_last_row, input(row, 0), 0u);
  auto path_1 =
      best_path_value_through_element(top_last_row, input(row, 1), 1u);
  auto top_paths = sorted_first_two(path_0, path_1);
  for (auto i = 2u; i < input.cols(); ++i) {
    auto path_i =
        best_path_value_through_element(top_last_row, input(row, i), i);
